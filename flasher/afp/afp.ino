// Arduino EEPROM Programmer (for the 39SF010, 39SF020, and 39SF040)


// Flash-invariant (port B bits 0,1)
// - between function calls, but maybe "violated" within a function
...
#define SET_LED(state)    bitWrite(PORTB, 5, state)   // Indicator LED on the Arduino nano board
...
void drv_id() {
  ...
  SET_OEN(LOW);
    ...
    uint8_t manid= drv_io_get();
    drv_addr_set(0x00001);
    uint8_t devid= drv_io_get();
  SET_OEN(HIGH);
  // Print the two IDs
  Serial.print("manid ");
  Serial.print(manid,HEX);
  if( manid==0xBF ) Serial.print(" Microchip");
  Serial.println();
  Serial.print("devid ");
  Serial.print(devid,HEX);
  if( devid==0xB5 ) Serial.print(" 39SF010");
  if( devid==0xB6 ) Serial.print(" 39SF020");
  if( devid==0xB7 ) Serial.print(" 39SF040");
  Serial.println();
...
void setup() {
  ...
  for( uint32_t a=addr; a<addr+16; a++ ) {
    drv_addr_set(a);
    uint8_t io= drv_io_get();
    if( io<10 ) Serial.print("0");
    Serial.print(io,HEX);
    Serial.print(" ");
  }
  Serial.println();
