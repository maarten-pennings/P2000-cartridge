...
The layout is available as two pdfs [front](pcb/P2000Cartridge-pcb-front.pdf) 
and [back](pcb/P2000Cartridge-pcb-back.pdf),
...
Warning, the socket diagram is the pin-out from the front side of the PCB 
("bottom view").
...
I did a quick "continuity" test; are all pads going where I think they 
need to go.

For a moment I believed I mirrored the flash socket.
That was due to the diagram from when I checked the [layout](#layout).
That diagram is mirrored, since I checked it in EasyEda with the front 
side visible. Now I was measuring at the backside.


## Assembly

Next step is assembly. That was easy. Just one socket, one DIL IC, 
2 capacitors, 5 resistors and a dip-switch. The copper pours on the 
PCB caused some soldering issues: the pad don't get hot enough.
...
- placed the flash socket even higher to make it fit in a standard 
  P2000 cartridge (the socket is the thickest, up in the cartridge 
  there is more depth).
